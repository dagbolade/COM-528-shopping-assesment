    ...
    function fetch_category()
            {
                ...
                xmlhttp.onreadystatechange = function ()
                {
                    ...
                    if (jsonar.length == 0)
                    {
                        alert("No doctor record found");
                    } else
                    {
                        var tab = "<table border=1 class='table table-bordered'>";
                        tab += "<thead><tr><td>category Name</td><td>Description</td><td>Photo</td><td>Remove</td></tr></thead>";
                        tab += "<tbody>";
                        for (var i = 0; i < jsonar.length; i++)
                        {
                            var obj = jsonar[i];
                            tab += "<tr><td>" + obj.category_name + "</td>";
                            tab += "<td>" + obj.category_desc + "</td>";
                          
                            tab+="<td><img src='" +obj.category_photo+ "'  style='width: 150px; height: 150px '/></td>";
                       tab +="<td><input type='button' id='confirm' value='Delete' class='btn btn-danger' onclick='confirm("+i+")'/></td>";
                ...
                xmlhttp.open("GET", "./fetch_category", true);
                xmlhttp.send();
            }
...
//                                window.location.href = "admin_home.jsp";
                    ...
                    // Step 2
                    xhttp.open("GET", "./delete_category?categoryname=" + obj.category_name, true);    // true --> async request
